...
# Iyzico internetten alışveriş deneyimini hem alıcılar hem de satıcılar için kolaylaştıran bir finansal teknolojiler
# şirketidir. E-ticaret firmaları, pazaryerleri ve bireysel kullanıcılar için ödeme altyapısı sağlamaktadır.
# 2020 yılının son 3 ayı için merchant_id ve gün bazında toplam işlem hacmi tahmini yapılması beklenmektedir.
...
# Unnamed: 0: Index bilgisini gösteren değişken
# transaction_date: İşlem tarihi
# merchant_id: Üye iş yerlerinin id'leri
# Total_Transaction: Toplam işlem sayısı
...
df = pd.read_csv("Time_Series/iyzico_data.csv")

# 'Unnamed: 0' değişkenini ver setinden siliyoruz.
...
# transaction_date değişkeninin tipini date'e çeviriyoruz
...
########################
# Lag/Shifted Features (Yt-1)
########################
"""
random_noise fonksiyonu ile veri setinde rastgele gürültü üretiyoruz. Bu fonksiyon ile amacımız, Lag/Shifted
değişkenlerini üretmeden önce veri setinde rastgele gürültü yaratarak aşırı öğrenmenin önüne geçmektir. Bu işlemi
uygularken veri setinin örüntüsüne (pattern) ve rassallığına zarar vermiyoruz.
"""

def random_noise(dataframe):
    return np.random.normal(scale=1.6, size=(len(dataframe)))   # Standart sapma 1.6 olarak belirlenmiştir.

df.head()

# Satış değişkenine göre yeni değişkenler üretiyoruz. Bu değişkenler satış değişkeninin önceki değerlerini taşıyacak.
# Bu feature'lar, mevsimsellik, trend ve diğer zamansal etkileri yakalamak için kullanılır.
...
########################
"""
Rolling Mean Features, bir zaman serisi veri setindeki her bir öğenin belirli bir zaman dilimi boyunca önceki öğelerine 
göre ortalama değerinin alınmasıdır. Bu feature'lar veri setindeki genel trendi daha iyi anlamaya yardımcı olur.
"""
...
########################
"""
Exponentially Weighted Mean Features, zaman serisi verilerindeki her bir öğenin önceki öğelerine göre ağırlıklı ortalama
değerinin alınmasıdır. Bu yöntem, her bir önceki öğenin ağırlığının, önceki öğeden uzaklaştıkça hızla azaldığı bir 
ağırlıklı ortalama hesaplaması yöntemidir. Bu nedenle, son öğeler daha yüksek ağırlığa sahip olur ve daha yeni verilerin
daha büyük etkisi olur.
"""
    ...
    for alpha in alphas:
        for lag in lags:
            dataframe['sales_ewm_alpha_' + str(alpha).replace(".", "") + "_lag_" + str(lag)] = \
                dataframe.groupby("merchant_id")['Total_Transaction']. \
                    transform(lambda x: x.shift(lag).ewm(alpha=alpha).mean())
    return dataframe

# Alpha değerlerinin düşmesiyle birlikte ortalamanın gerçek değere daha da yakın olmasını bekleriz.
...
# MAPE: mean absolute percentage error (oransal prtalama mutlak hata)
# SMAPE: Symmetric mean absolute percentage error (adjusted MAPE) (Simetrik Oransal Ortalama Mutlak Hata)
"""
SMAPE (Simetrik Oransal Ortalama Mutlak Hata), bir tahmin modelinin gerçek değerlerden yüzde olarak ne kadar uzak 
olduğunu hesaplamak için kullanılan bir hata ölçüsüdür. SMAPE, gerçek değerlerin ve öngörülen değerlerin farklarının 
mutlak değerlerinin, gerçek ve öngörülen değerlerin toplamının yarısına bölünmesiyle hesaplanır.
"""
...
# Bağımsız değişkenlerden zaman serisi modelinde kullanılmayacak olan değişkenleri çıkartıyoruz.
